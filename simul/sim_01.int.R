...
################################################################################
# SIMULATION STUDY
# PERFORM THE SIMULATIONS (INTERNAL METHOD)
################################################################################
...
# LOOP ACROSS RANDOMLY SIMULATED DATA
  ...
  for(i in seq(nsim)) {
    ...
    L <- matrix(0:40,nrow(Q),ncol(Q),byrow=TRUE)
...
# FIT GAM-REML
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10),family=poisson,method='REML')
    # STORE THE TIME
    ...
    # PREDICT
    cp <- crosspred("Q",model,from=0,to=10,by=0.25,bylag=1,cen=cen[j])
...
# FIT GAM-AIC
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10),family=poisson,method='GCV.Cp')
    # STORE THE TIME
    timemodel[[p]][i,j] <- (proc.time()-mtime)[3]
...
# FIT GAMcr
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10,xt=list(bs="cr")),family=poisson,
      method='REML')
    # STORE THE TIME
    timemodel[[p]][i,j] <- (proc.time()-mtime)[3]
...
# FIT GAMps21
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10,m=list(c(2,2),c(2,1))),family=poisson,
      method='REML')
    # STORE THE TIME
    timemodel[[p]][i,j] <- (proc.time()-mtime)[3]
...
# FIT GAM-last
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10,xt=list(addSlag=rep(0:1,c(6,4)))),
...
# FIT GAM-quad
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    C <- do.call(onebasis,list(x=0:40,fun="ps",int=T))
    D <- diff(diag(41),diff=2)
    P <- diag((0:38)^2)
    Slag2 <- t(C)%*%t(D)%*%P%*%D%*%C
    model <- gam(y~s(Q,L,bs="cb",k=10,fx=c(F,T),xt=list(addSlag=Slag2)),
      family=poisson,method='REML')
    # STORE THE TIME
    timemodel[[p]][i,j] <- (proc.time()-mtime)[3]
...
# FIT GAM-exp
    ...
    # RUN THE MODEL
    mtime <- proc.time()
    model <- gam(y~s(Q,L,bs="cb",k=10,fx=c(F,T),xt=list(addSlag=exp(0:9))),
      family=poisson,method='REML')
    # STORE THE TIME
    timemodel[[p]][i,j] <- (proc.time()-mtime)[3]
